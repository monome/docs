...
We sell full replacement PCBs for standard norns. E-mail help@monome.org

We no longer service norns shields, or have replacement PCBs. PCBs are available through various resellers as the design is open-source.

This is a slightly tricky DIY repair. The encoders are sensitive to heat and require quick, confident soldering.

The replacement part is PEC11R-4015F-N0024.
...
1. Boot norns first, then plug in the nub.
...
5. Plug the WiFi nub into a non-norns computer (laptop/desktop ; MacOS/Windows/Linux) and confirm that the nub functions as expected. If your nub is defective, please email help@monome.org for a replacement.

6. If you are prompted to update the nub's drivers, please do so. Even if there are no updates available, sometimes the simple task of searching for an update resolves connectivity issues. When this process completes, plug the nub back into norns.

7. If norns is still unable to connect to WiFi, connect the power cable to your non-norns computer and follow the `serial` steps outlined [here](/docs/norns/advanced-access/#serial). Once you perform this serial login, try executing `nmtui` for a graphical interface of the WiFi utilities, which may have better luck connecting to a network:
   
    ![](/docs/norns/image/terminal-nmtui-main.png)

8. If you are still unable to connect, please email help@monome.org with the following information:
   
   - Whether your nub was able to successfully connect with a non-norns computer
   - Whether the nub seems active if plugged in after norns boots
