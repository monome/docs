...
* [Reach](/docs/modular/ansible/teletype) (Teletype) further ecosystem output and input.
...
Be careful not to misalign the connector left-to-right either ¡V while Ansible is protected against incorrect connection, doing so may damage other modules in your Eurorack case.
...
* The key to the left which is separate from the other two is the
  panic key ¡V press it to restore your saved tuning table from flash,
