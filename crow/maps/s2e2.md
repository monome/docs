...
to start, Trent shorthands `sequins` so we don't have to write 7 characters every time we want to employ the `sequins` library. since `sequins` is a globally known library inside of crow, we can simply assign a variable `s` to all of its functions:
